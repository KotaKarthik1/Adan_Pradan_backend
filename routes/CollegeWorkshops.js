...
router.post("/addworkshops", async (req, res) => {
  try {
    ...
    for (const workshop of workshops) {
      // Check if workshop with same title and date already exists
      const existingWorkshop = await WorkshopData.findOne({
        workshopTitle: workshop.workshopTitle,
        workshopDate: workshop.workshopDate,
      });

      if (existingWorkshop) {
        // Workshop already exists, send a message
        return res.status(400).json({
          error: `Workshop with title '${workshop.workshopTitle}' and date '${workshop.workshopDate}' already exists`,
        });
      }
    ...
    // Find the ClgInfo document by collegeId
    const college = await ClgInfo.findOne({ _id: collegeId });
    if (!college) {
      return res.status(404).json({ error: "College not found for the given collegeId" });
    }
